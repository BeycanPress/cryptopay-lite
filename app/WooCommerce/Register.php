        ...
        if (function_exists('WC')) {
            ...
            Hook::addFilter('check_order_woocommerce', function (object $order) {
                if (!wc_get_order($order->id)) {
                    Response::error(
                        esc_html__('The relevant order was not found!', 'cryptopay_lite'),
                        'ORDER_NOT_FOUND'
                    );
                }
            ...
            Hook::addAction('payment_started_woocommerce', function (object $data): void {
                $order = wc_get_order($data->order->id);
                $order->update_status('wc-on-hold');
            });

            Hook::addAction('payment_finished_woocommerce', function (object $data): void {
                if ($order = wc_get_order($data->order->id)) {
                    ...
                    $order->update_meta_data(
                        ...
                        Services::toString(
                            ...
                            $data->order->paymentCurrency->decimals
                        )
                    );
